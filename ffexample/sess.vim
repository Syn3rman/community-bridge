...
nnoremap <silent> ì :TmuxNavigateRight
nnoremap <silent> ë :TmuxNavigateUp
nnoremap <silent> ê :TmuxNavigateDown
nnoremap <silent> è :TmuxNavigateLeft
cnoremap kj 
inoremap kj 
let &cpo=s:cpo_save
unlet s:cpo_save
...
silent only
silent tabonly
cd ~/Documents/community_bridge/ffexample
...
wincmd w
argglobal
if bufexists("server/server.go") | buffer server/server.go | else | edit server/server.go | endif
let s:cpo_save=&cpo
set cpo&vim
inoremap <buffer> <silent> <M-n> :call AutoPairsJump()a
inoremap <buffer> <silent> <expr> <M-p> AutoPairsToggle()
...
exe 'vert 2resize ' . ((&columns * 67 + 67) / 135)
tabnext
edit fluentforward/fluentforward.go
set splitbelow splitright
wincmd t
set winminheight=0
set winheight=1
...
tabnext 1
set stal=1
badd +1 client/client.go
badd +0 server/server.go
badd +0 fluentforward/fluentforward.go
